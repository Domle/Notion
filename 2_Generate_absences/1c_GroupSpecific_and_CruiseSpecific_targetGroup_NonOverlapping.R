...
# deriksson@ethz.ch, 9th of September 2023 ----------------------------------------------
...
for(d in seq_along(nms.sets)){
	...
	pres_abs_results <- foreach (s = 1:length(df.taxa.obs$taxon), .packages = c('dismo', 'classInt'))%dopar%{
		...
		if (ob < min.obs){
            ...
            table_abs3 <- data.frame(table_abs2, prob = sample(random_num, dim(table_abs1)[1], replace = TRUE)) # Adds column with random number between 0 and 1 (with steps 0.01)
            ...
            } # end of loop across absence_group
