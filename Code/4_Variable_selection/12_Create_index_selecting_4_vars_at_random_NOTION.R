...
# Author: rdamiano@ethz.ch - Damiano Righetti
# $ Date 2018-01-15, updated 2020-06-10

# The index defines five models à 4 different predictors, each, to build an SDM ensemble
...
# c) selecting variables from the top ten, and if needed, subsequent ranks
...
# Directories
input.dir <- "../7_Generate_Absences/Output_11"
output.dir <- "./Output_12_Index"

# Preparatory: data frame with the correlates for each variable
my.corr  <- read.csv("~/Desktop/Data/8_Variable_Selection/Output_8/Global_corr_table_spearman.csv")
names(my.corr)
rownames(my.corr) <- names(my.corr[2:28])
my.corr$X <- NULL

### =========================================================================
### A. Sites - based approach. Select variables for the first best model across species
### =========================================================================

# Get data on species and variable ranking
dat.ran <- read.csv("~/Desktop/Data/8_Variable_Selection/Output_11/Analysis ranking/Species_minobs_14_sites_tot_bg.csv") # Results on taxa and species specific to NOTION
...
for( s  in  1:length(na.omit(unique(dat.ran$taxon))) ){
					...
					if(model == "glm") {dat.spp.raw <- dat.ran[          which(dat.ran$taxon == na.omit(unique(dat.ran$taxon))[s] ) [2]                ,   3:28 ]} # The second position is site glm
					if(model == "gam") {dat.spp.raw <- dat.ran[          which(dat.ran$taxon == na.omit(unique(dat.ran$taxon))[s] ) [4]                ,   3:28 ]} # The fourth position is site gam
					if(model == "rf") {dat.spp.raw <- dat.ran[          which(dat.ran$taxon == na.omit(unique(dat.ran$taxon))[s] ) [6]                ,   3:28 ]} # The sixth position is site rf
					if(model == "ensemble") {dat.spp.raw <- dat.ran[           max(which(dat.ran$taxon == na.omit(unique(dat.ran$taxon))[s] ))+1                  ,   3:28 ]} # The ensemble mean rank is rank seven (the "max plus one")
					#
					...
					for(u in 1:5){
							...
							my.choice[1] <- as.character(my.potential.vars[sample(nrow(my.potential.vars), 1, prob= my.potential.vars$weight  ), "var"])
							...
							while(   length(my.choice)  < 4  ){  # ** CRITICAL DECISION: NUMBER OR PREDICTORS
									...
									# The remaining positions that potentially provide one predictor to the memer are 
									valid.positions <- df.var[which(df.var$check.vector < 2 & df.var$rank < 11), ]
									...
									columns.with.values.greater.zero.point.seven <- which(apply(df.corr.relevant, 2, max, na.rm=T) >0.7)
									...
									valid.pos <- valid.positions[   -which(valid.positions$var%in%variables.correlated)   ,   ]} # 
											...
											if(nrow(valid.pos) == 0) {
													...
													i = 11 # we start with the variable that is ranked 11
													...
													if(   any (   corr.subset       > 0.7, na.rm =T   ) == TRUE ) {i = i+1; next} else {
...
# A few stats (ensemble mean approach)
...
# Save
fln <- paste0("~/Desktop/Data/8_Variable_Selection/Output_12_Index/Species_minobs_14_frames_5_members_a_4_predictors_(", model, ")_tot_bg_NOTION.RData")
save(lisi.frames, file = fln)
			
fln <- paste0("~/Desktop/Data/8_Variable_Selection/Output_12_Index/Species_minobs_14_index_5_members_a_4_predictors_(",model,")_NOTION.csv")
