...
# This is the version by - C.RANDIN - UNIBAS - March 2013; randomForest uses n = 1000 trees and is not accounting for imbalanced data in this basic version.
### Help page: http://finzi.psych.upenn.edu/library/ecospat/html/ecospat.cv.rf.html
...
# For testing
# rf.obj <- mod_full
# data.cv <- as.data.frame(cbind( dat[ , c("obs")], dat[ , which(names(dat)%in%vrs)]))
# K <- 8
# cv.lim <- 1
# jack.knife <- F
